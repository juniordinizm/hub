...
function AlertDialogOverlay({
  ...
  return (
    <AlertDialogPrimitive.Overlay
      className={cn(
        "data-open:fade-in-0 data-closed:fade-out-0 fixed inset-0 z-50 bg-black/30 duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] data-closed:animate-out data-open:animate-in supports-backdrop-filter:backdrop-blur-sm",
        className
      )}
  ...
  return (
    <AlertDialogPortal>
      ...
      <AlertDialogPrimitive.Content
        className={cn(
          "group/alert-dialog-content data-open:fade-in-0 data-open:zoom-in-95 data-closed:fade-out-0 data-closed:zoom-out-95 fixed top-1/2 left-1/2 z-50 grid w-full -translate-x-1/2 -translate-y-1/2 gap-6 rounded-4xl bg-popover p-6 text-popover-foreground shadow-[0_8px_30px_rgb(0,0,0,0.12)] outline-none ring-1 ring-foreground/5 duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] data-[size=default]:max-w-xs data-[size=sm]:max-w-xs data-closed:animate-out data-open:animate-in data-[size=default]:sm:max-w-md dark:ring-foreground/10",
          className
        )}
        data-size={size}
